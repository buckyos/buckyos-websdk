...
export class ReplHistory {
  ...
  async load(): Promise<string[]> {
    try {
      this.#entries = (await getHost().readTextFile(this.path))
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean)
        .slice(-this.limit)
  ...
  async add(line: string, command?: RegisteredCommand): Promise<void> {
    ...
    const host = getHost()
    await host.mkdir(host.path.dirname(this.path), { recursive: true, mode: 0o700 })
    await host.writeTextFile(this.path, `${this.#entries.join('\n')}\n`, { mode: 0o600 })
    if (host.platform !== 'windows') await host.chmod(this.path, 0o600)
  }
}
