...
import { applyImplicitDeviceIdentity, identityCandidateView } from './identity.ts'
...
export const VERSION = TOOL_VERSION
...
export class BuckyOSToolApplication {
  ...
  async run(argv: string[]): Promise<number> {
    let fallbackTraceId: string = crypto.randomUUID()
    try {
      ...
      if (invocation.global.version) {
        ...
        if (invocation.global.verbose) {
          ...
          let identityCandidates: Record<string, unknown>
          try {
            const setup = await resolveConfig(invocation.global, this.#environment, {
              cwd: this.#cwd,
              homeDir: this.#homeDir,
            })
            setup.resolved = await applyImplicitDeviceIdentity(
              setup.resolved,
              this.#environment,
            )
