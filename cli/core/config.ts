...
export class ConfigStore {
  ...
  async #readJson<T>(path: string, missing: T | undefined): Promise<T> {
    try {
      const parsed = JSON.parse(await getHost().readTextFile(path)) as unknown
      ...
      if (isHostError(error, 'NotFound') && missing !== undefined) return missing
  ...
  async #atomicWrite(path: string, value: unknown): Promise<void> {
    const host = getHost()
    await host.mkdir(dirname(path), { recursive: true, mode: 0o700 })
    const temporary = `${path}.tmp-${host.pid}-${crypto.randomUUID()}`
    const body = `${JSON.stringify(value, null, 2)}\n`
    try {
      await host.writeTextFile(temporary, body, { mode: 0o600, createNew: true })
      await host.rename(temporary, path)
      if (host.platform !== 'windows') await host.chmod(path, 0o600)
    } catch (error) {
      try {
        await host.remove(temporary)
      } catch (cleanupError) {
        void cleanupError
      }
      throw error
...
export function resolveConfigRoot(
  ...
  cwd = getHost().cwd(),
  homeDir = environment.HOME ?? environment.USERPROFILE ?? getHost().homeDir(),
): string {
...
export function readEnvironment(): Environment {
  ...
  return Object.fromEntries(
    ENVIRONMENT_NAMES.map((
      name,
    ) => [name, allowed.includes(name) ? getHost().env(name) : undefined]),
  )
}
