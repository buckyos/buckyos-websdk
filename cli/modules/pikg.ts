...
import { PROTOCOL_VERSION, SDK_VERSION, TOOL_VERSION } from '../version.ts'
import { identityCandidateView } from '../core/identity.ts'

const basename = (path: string) => getHost().path.basename(path)
const dirname = (path: string) => getHost().path.dirname(path)
const isAbsolute = (path: string) => getHost().path.isAbsolute(path)
const join = (...parts: string[]) => getHost().path.join(...parts)
const relative = (from: string, to: string) => getHost().path.relative(from, to)
...
function reproducibleBuildTimestamp(): number {
  const configured = getHost().env('SOURCE_DATE_EPOCH')
  if (configured === undefined) return Math.floor(Date.now() / 1000)
...
async function doctorCommand(
  ...
  const metaDir = resolveFromCwd(ctx, metaInput)
  const checks: Array<Record<string, unknown>> = []
  let appMeta: Record<string, unknown> | undefined
  let pikgMeta: Record<string, unknown> | undefined
  for (
    const [name, path] of [
    ...
    try {
      ...
      if (name === 'app_metadata') appMeta = value
      else pikgMeta = value
  ...
  if (appMeta && pikgMeta) {
    try {
      ...
      for (const subpackage of Object.values(pikg.sub_pkgs)) {
        if (subpackage.source.type !== 'path') continue
        const source = resolve(metaDir, subpackage.source.path)
        try {
          await getHost().lstat(source)
        } catch (error) {
          if (isHostError(error, 'NotFound')) missingSources.push(source)
          else throw error
        }
      }
      checks.push({
  ...
  const profile = profileName ? await ctx.configStore.readProfile(profileName) : undefined
  const host = getHost()
  return {
    ...
    ...policyView(host),
    distribution_manifest: distributionManifestView(await readDistributionManifest(host)),
    target: {
...
async function buildCommand(
  ...
  for (const subpackage of Object.values(pikgMeta.sub_pkgs)) {
    if (subpackage.source.type === 'path') {
      sourcePaths.push(await getHost().realPath(resolve(metaDir, subpackage.source.path)))
    }
  }
  ...
  try {
    ...
    for (
      ...
      if (subpackage.source.type === 'path') {
        const sourcePath = await getHost().realPath(resolve(metaDir, subpackage.source.path))
...
async function initializeMetaDirectory(
  ...
  try {
    const stat = await getHost().lstat(metaDir)
  ...
  try {
    ...
    if (existedEmpty) await getHost().remove(metaDir)
    try {
      await getHost().rename(temporary, metaDir)
    } catch (error) {
      if (existedEmpty) await getHost().mkdir(metaDir)
      throw error
    }
  } catch (error) {
    await removeTreeIfExists(temporary)
...
async function validateReplaceableDist(distDir: string, expectedMetaRootId: string): Promise<void> {
  try {
    ...
    if (isHostError(error, 'NotFound')) return
    throw error
  }
...
async function validateManagedDist(
  ...
  try {
    ...
    for (const name of Object.keys(manifest.generated_files)) {
      const info = await getHost().lstat(join(distDir, name))
...
async function realDirectory(path: string, label: string): Promise<string> {
  ...
  const stat = await getHost().stat(real)
...
function pathsOverlap(left: string, right: string): boolean {
  ...
  return leftToRight === '' || !leftToRight.startsWith(`..${separator}`) && leftToRight !== '..' ||
    !rightToLeft.startsWith(`..${separator}`) && rightToLeft !== '..'
}
...
function statIdentity(value: HostFileInfo): string {
...
function displaySource(projectDir: string, source: string): string {
  ...
  return display && !display.startsWith(`..${getHost().path.sep}`) && display !== '..'
