...
export async function digestFile(path: string): Promise<FileDigest> {
  ...
  return { size, sha256: sha.digestHex(), crc32: (crc ^ 0xffffffff) >>> 0 }
}
...
export async function createDeterministicTarGz(
  ...
  const tar = await getHost().open(tarPath, { createNew: true, write: true, mode: 0o600 })
  try {
...
export async function writeStoredZip(
  ...
  await getHost().mkdir(dirname(destination), { recursive: true })
  const output = await getHost().open(destination, { createNew: true, write: true, mode: 0o600 })
  let offset = 0
...
export async function openZip(path: string): Promise<OpenZip> {
  const stat = await getHost().stat(path)
...
async function collectTarSources(root: string): Promise<TarSource[]> {
  ...
  async function walk(physicalDirectory: string, archivePrefix: string): Promise<void> {
    const realDirectory = await getHost().realPath(physicalDirectory)
    ...
    try {
      const children = [] as HostDirEntry[]
      for (const child of await getHost().readDir(realDirectory)) children.push(child)
...
function fileIdentity(link: HostFileInfo, realPath: string, target: HostFileInfo): string {
...
function assertWithin(root: string, candidate: string, label: string): void {
  ...
  if (
    path === '..' || path.startsWith(`..${getHost().path.sep}`) ||
...
async function writeAll(file: HostFile, bytes: Uint8Array): Promise<void> {
  let offset = 0
  while (offset < bytes.length) offset += await file.write(bytes.subarray(offset))
}

async function copyFileToWriter(path: string, output: HostFile): Promise<void> {
...
async function readAt(file: HostFile, offset: number, length: number): Promise<Uint8Array> {
