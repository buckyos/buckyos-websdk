...
describe('golden fixtures (generated by buckycli, see fixtures README)', () => {
  ...
  test('owner document zone binding v2 transitions are explicit and idempotent', async () => {
    const ownerKeys = getDevTestKeyPairById('devtest')
    const ownerDoc = newOwnerDocument({
      did: 'did:bns:lzc',
      name: 'lzc',
    ...
    expect(ownerDoc.zone_binding_model_version).toBe(ZONE_BINDING_MODEL_VERSION)
    expect(ownerDocumentGetZoneBindingState(ownerDoc, 'did:bns:zone1')).toBe('unbound_v2')
    const unboundJwt = await encodeOwnerDocument(ownerDoc, ownerKeys.privateKeyPem)
    const unboundPayload = decodeJwtClaimWithoutVerify(unboundJwt)
    expect(unboundPayload.zone_binding_model_version).toBe(ZONE_BINDING_MODEL_VERSION)
    expect(unboundPayload.binded_zone_list).toBeUndefined()

    ownerDoc.service = [{
      id: 'did:bns:lzc#profile',
      type: 'Profile',
      serviceEndpoint: 'https://profile.example.com/lzc',
    }]
    ownerDocumentSetDefaultZoneDid(ownerDoc, 'did:bns:zone1')
    const firstBind = JSON.stringify(ownerDoc)
    ownerDocumentSetDefaultZoneDid(ownerDoc, 'did:bns:zone1')
    expect(JSON.stringify(ownerDoc)).toBe(firstBind)
    expect(ownerDocumentGetZoneBindingState(ownerDoc, 'did:bns:zone1')).toBe('bound_v2')

    ownerDocumentSetDefaultZoneDid(ownerDoc, 'did:web:zone2.example.com')
    expect(ownerDoc.binded_zone_list).toEqual(['did:web:zone2.example.com', 'did:bns:zone1'])
    expect(ownerDocumentRemoveBoundZone(ownerDoc, 'did:bns:zone1')).toBe(true)
    expect(ownerDocumentGetDefaultZoneDid(ownerDoc)).toBe('did:web:zone2.example.com')

    ownerDocumentSetDefaultZoneDid(ownerDoc, 'did:bns:zone1')
    expect(ownerDocumentRemoveBoundZone(ownerDoc, 'did:bns:zone1')).toBe(true)
    expect(ownerDocumentGetDefaultZoneDid(ownerDoc)).toBe('did:web:zone2.example.com')
    expect(ownerDoc.service?.find(service => service.id === 'did:bns:lzc#lastDoc')?.serviceEndpoint)
      .toBe('https://zone2.example.com/resolve/did:bns:lzc')

    expect(ownerDocumentRemoveBoundZone(ownerDoc, 'did:web:zone2.example.com')).toBe(true)
    expect(ownerDoc.binded_zone_list).toEqual([])
    expect(ownerDoc.service?.some(service => service.id === 'did:bns:lzc#lastDoc')).toBe(false)
    expect(ownerDoc.service?.some(service => service.id === 'did:bns:lzc#profile')).toBe(true)
    expect(ownerDocumentGetZoneBindingState(ownerDoc, 'did:web:zone2.example.com')).toBe('unbound_v2')
    expect(ownerDocumentRemoveBoundZone(ownerDoc, 'did:web:zone2.example.com')).toBe(false)

    const legacy = { ...ownerDoc }
    delete legacy.zone_binding_model_version
    expect(ownerDocumentGetZoneBindingState(legacy, 'did:bns:lzc')).toBe('legacy')
    expect(ownerDocumentGetZoneBindingState({ ...ownerDoc, zone_binding_model_version: 3 }, 'did:bns:lzc'))
