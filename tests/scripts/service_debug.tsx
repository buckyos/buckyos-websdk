...
function printUsage(): never {
  console.error(
    [
      'Usage:',
      '  service_debug <app_service_name> <owner_user_id> [--port <port>] [--node-id <node_id>] [--detach] [--print-env-json]',
      '',
      'Example:',
...
async function main() {
  try {
    ...
    if (options.printEnvJson) {
      console.log(JSON.stringify({
        runtime: launch.runtime,
        appId: options.appId,
        ownerUserId: options.ownerUserId,
        nodeId: launch.nodeId,
        servicePort: launch.servicePort,
        env: launch.env,
      }, null, 2))
      return
    }
