...
describe('AuthClient in SSO browser environment', () => {
  it('builds the SSO login URL with the current page as default redirect target', () => {
    ...
    expect(authClient.buildLoginURL()).toBe(
      'http://sys.test.buckyos.io/sso/login?client_id=demo-app&redirect_uri=http%3A%2F%2Flocalhost%2F&response_type=token',
    )
  })
  ...
  it('login redirects the current window instead of opening a popup', async () => {
    ...
    expect(navigate).toHaveBeenCalledWith(
      'http://sys.test.buckyos.io/sso/login?client_id=demo-app&redirect_uri=https%3A%2F%2Fapp.test%2Flogin%2Fcallback&response_type=token',
    )
