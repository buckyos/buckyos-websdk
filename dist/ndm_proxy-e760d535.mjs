...
const defaultFetcher$1 = async (input, init2) => {
...
class kRPCClient {
  ...
  buildRequest(method, params, seq, sessionToken, traceId) {
    let sys;
    if (traceId) {
  ...
  // a session token.
  parseResponseSys(sys, currentSeq) {
...
class SystemConfigClient {
  ...
  setConfigCache(key, value, version2) {
    ...
    return previous.value !== value || previous.version !== version2;
  }
...
function hasNodeRuntime$1() {
  var _a2;
  const runtimeProcess = globalThis.process;
  return Boolean((_a2 = runtimeProcess == null ? void 0 : runtimeProcess.versions) == null ? void 0 : _a2.node);
}
...
class AppClientRuntimeProfile extends ManagedSessionRuntimeProfile {
  ...
  getZoneGatewayServiceURL(runtime, servicePath) {
    const zoneHost = trimToNull$2(runtime.getZoneHostName());
    if (!zoneHost) {
  ...
  shouldSkipVerifyHubRenewal(runtime) {
    return !trimToNull$2(runtime.getZoneHostName()) && !runtime.getConfiguredVerifyHubServiceUrl();
  }
...
class BuckyOSRuntime {
  constructor(config) {
    const normalizedOwnerUserId = trimToNull$2(config.ownerUserId) ?? trimToNull$2(config.userid);
    this.config = {
      ...DEFAULT_CONFIG,
      ...config,
      appId: config.appId,
  ...
  getConfiguredVerifyHubServiceUrl() {
    return trimToNull$2(this.config.verifyHubServiceUrl);
  }
  ...
  async resolveZoneHostFromLocalConfig() {
    ...
    if (trimToNull$2(this.config.zoneHost)) {
      return;
    }
  ...
  async requestBrowserSessionToken(authStateRevision) {
    ...
    try {
      ...
      const sessionToken = trimToNull$2(
  ...
  async getPrivateKeySearchRoots() {
    ...
    for (const item of this.config.privateKeySearchPaths ?? []) {
      const trimmed = trimToNull$2(item);
      if (trimmed) {
        roots.push(trimmed);
      }
    }
    const explicitClientDir = trimToNull$2(env.BUCKYOS_APP_CLIENT_DIR);
    if (explicitClientDir) {
      roots.push(explicitClientDir);
    }
    const homeDir = trimToNull$2(env.HOME) ?? trimToNull$2(env.USERPROFILE) ?? trimToNull$2((_a2 = os.homedir) == null ? void 0 : _a2.call(os));
    if (homeDir) {
      roots.push(path.join(homeDir, ".buckyos"));
      roots.push(path.join(homeDir, ".buckycli"));
    }
    const rootDir = trimToNull$2(this.config.rootDir) ?? trimToNull$2(env.BUCKYOS_ROOT) ?? "/opt/buckyos";
    roots.push(rootDir);
  ...
  async getBuckyOSRootDir() {
    const env = getProcessEnv();
    return trimToNull$2(this.config.rootDir) ?? trimToNull$2(env.BUCKYOS_ROOT) ?? "/opt/buckyos";
  }
  ...
  async deviceKeyPathCandidates(identityDir, deviceDid) {
    ...
    if (deviceDid) {
      try {
        ...
        const env = getProcessEnv();
        const securityRoot = trimToNull$2(env.BUCKYOS_SECURITY_ROOT) ?? path.join(trimToNull$2(env.BUCKYOS_ROOT) ?? "/opt/buckyos", "security");
  ...
  async tryLoadUserSigningMaterial(userId, roots) {
    ...
    for (const root of roots) {
      ...
      try {
        ...
        const configUserId = (_a2 = ownerDoc == null ? void 0 : ownerDoc.name) == null ? void 0 : _a2.trim();
  ...
  async tryResolveDeviceNameFromSearchRoots(roots) {
    const path = await importNodeModule$1("node:path");
    const env = getProcessEnv();
    const fromEnv = trimToNull$2(env.BUCKYOS_THIS_DEVICE_NAME);
    if (fromEnv) {
      return fromEnv;
    }
    for (const key of ["BUCKYOS_THIS_DEVICE", "BUCKYOS_THIS_DEVICE_INFO"]) {
      const raw = trimToNull$2(env[key]);
      if (!raw) {
        continue;
      }
      try {
  ...
  async tryResolveZoneHostFromSearchRoots(roots) {
    ...
    const path = await importNodeModule$1("node:path");
    const env = getProcessEnv();
    const fromEnv = trimToNull$2(env.BUCKYOS_ZONE_HOST);
    if (fromEnv) {
      return fromEnv;
    }
    for (const root of roots) {
    ...
    for (const root of roots) {
      ...
      try {
        const raw = await fs.readFile(userConfigPath, "utf8");
        const ownerDoc = parseBuckyOSOwnerDocument(JSON.parse(raw));
        const zoneHost = resolveZoneHostFromDid((_a2 = ownerDoc == null ? void 0 : ownerDoc.binded_zone_list) == null ? void 0 : _a2[0]);
        if (!zoneHost) {
          continue;
        }
        return zoneHost;
  ...
  getConfiguredSystemConfigServiceUrl() {
    return trimToNull$2(this.config.systemConfigServiceUrl);
  }
  resolveAppServiceGatewayHost() {
    return trimToNull$2(getProcessEnv()[BUCKYOS_HOST_GATEWAY_ENV]) ?? DEFAULT_DOCKER_HOST_GATEWAY;
  }
...
const defaultFetcher = async (input, init2) => {
...
function getBuckyApiBridge() {
  if (typeof window === "undefined") {
    return null;
  }
  const bridge = window.BuckyApi;
  return bridge && typeof bridge === "object" ? bridge : null;
}
async function getCurrentWalletUserFromHost() {
  const bridge = getBuckyApiBridge();
  const getCurrentUser = bridge == null ? void 0 : bridge.getCurrentUser;
  if (typeof getCurrentUser !== "function") {
    return null;
  }
  const result = await getCurrentUser.call(bridge);
...
function detectHostRuntimeType() {
  ...
  const runtimeProcess = globalThis.process;
  if ((_a2 = runtimeProcess == null ? void 0 : runtimeProcess.versions) == null ? void 0 : _a2.node) {
...
function getActiveRuntimeType() {
  var _a2;
  return ((_a2 = activeRuntimeContext.runtime) == null ? void 0 : _a2.getConfig().runtimeType) ?? detectHostRuntimeType();
}
...
class BuckyOSSDK {
  ...
  getBuckyOSConfig() {
    var _a2;
    return ((_a2 = this.currentRuntime) == null ? void 0 : _a2.getConfig()) ?? null;
  }
  getRuntimeType() {
  ...
  async loginByPassword(username, password) {
    ...
    const appInstanceId = (_a2 = this.currentRuntime) == null ? void 0 : _a2.getAppInstanceId();
    if (!appInstanceId) {
  ...
  walletSignWithActiveDid(payloads) {
    ...
    return (async () => {
      ...
      if (result.code === 0) {
        return {
          signatures: Array.isArray((_a2 = result.data) == null ? void 0 : _a2.signatures) ? result.data.signatures : [],
  ...
  buildRuntimeConfig(appid, config) {
    ...
    if (this.target === "node") {
      return {
        ...
        zoneHost: trimToNull$1(getNodeEnv().BUCKYOS_ZONE_HOST) ?? ""
      };
    }
    if (isBrowserRuntime()) {
      return {
  ...
  detectEnvironmentRuntimeType() {
    ...
    if (this.target === "node") {
      ...
      if ((_a2 = runtimeProcess2 == null ? void 0 : runtimeProcess2.versions) == null ? void 0 : _a2.node) {
...
class BnsClientError extends Error {
  ...
  get code() {
    ...
    if (this.kind === "registry") {
      return ((_a2 = this.info) == null ? void 0 : _a2.code) ?? "UNKNOWN_BNS_ERROR";
    }
...
const dString = (value, path) => typeof value === "string" ? value : fail(path, "expected string");
const dBool = (value, path) => typeof value === "boolean" ? value : fail(path, "expected boolean");
const dU64 = (value, path) => typeof value === "number" && Number.isInteger(value) && value >= 0 ? value : fail(path, "expected u64");
const dU32 = (value, path) => typeof value === "number" && Number.isInteger(value) && value >= 0 && value <= 4294967295 ? value : fail(path, "expected u32");
const dBytes = (value, path) => {
  if (!Array.isArray(value)) {
...
function dEnum(...values) {
  return (value, path) => typeof value === "string" && values.includes(value) ? value : fail(path, `expected one of ${values.join("|")}`);
}
function dArray(inner) {
  return (value, path) => {
    if (!Array.isArray(value)) {
...
function withMissing(inner, onMissing) {
  const decoder = (value, path) => inner(value, path);
  decoder.onMissing = onMissing;
  return decoder;
}
function dOptional(inner) {
  return withMissing((value, path) => value === null ? null : inner(value, path), () => null);
}
const dU64Default = withMissing(dU64, () => 0);
function dStruct(typeName, fields) {
  ...
  return (value, path) => {
    ...
    for (const fieldName of fieldNames) {
      const decoder = fields[fieldName];
      const fieldPath = `${path}.${fieldName}`;
      const fieldValue = source[fieldName];
      if (fieldValue === void 0 && !(fieldName in source)) {
...
const dNameStatus = dEnum("available", "active", "expired", "released", "tombstoned");
const dDocumentStatus = dEnum("missing", "active", "revoked", "expired", "migrated", "tombstoned");
const dAliasKind = dEnum("none", "alias", "migrated_to", "canonical");
const dReleaseMode = dEnum("release_after_grace", "tombstone_forever");
const dPrincipalKind = dEnum("unset", "chain_account", "bns_name");
const dOwnerSource = dEnum("none", "asset_owner_fallback", "explicit_semantic_owner", "parent_inherited");
const dAuthorityKeyStatus = dEnum("missing", "active", "revoked", "expired");
const dTxExecutionState = dEnum("not_found", "pending", "succeeded", "reverted");
const dPrincipal = dStruct("Principal", {
  kind: dPrincipalKind,
  value: dString
});
const dNameState = dStruct("NameState", {
  name: dString,
  asset_owner: dString,
...
const dOwnerResolution = dStruct("OwnerResolution", {
  effective_owner: dPrincipal,
  source: dOwnerSource,
  authority_root: dString,
  authority_seq: dU64
});
const dAuthoritySetState = dStruct("AuthoritySetState", {
  name: dString,
  authority_seq: dU64,
  authority_root: dString,
  active_key_count: dU32
});
const dAuthorityKey = dStruct("AuthorityKey", {
  kid: dString,
...
const dDocumentRef = dStruct("DocumentRef", {
...
const dDocumentState = dStruct("DocumentState", {
  name: dString,
  doc_type: dString,
  version: dU64,
...
const dResolveResult = dStruct("ResolveResult", {
...
const dBnsNamePage = dStruct("BnsNamePage", {
  names: dArray(dString),
  next_cursor: dOptional(dString)
});
const dBnsTxState = dStruct("BnsTxState", {
  tx_hash: dString,
  state: dTxExecutionState,
  block_number: dOptional(dU64),
  confirmations: dU64
});
const dSubmitRawTxResp = dStruct("BnsSubmitRawTxResp", {
  tx_hash: dString
});
const dLogCheckpoint = dStruct("LogCheckpoint", {
  log_root: dString,
  last_seq: dU64,
  issued_at: dU64,
  issuer: dPrincipal,
...
const REGISTRY_EVENT_DATA_DECODERS = {
...
const dRegistryEvent = (value, path) => {
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
  ...
  const dataDecoder = REGISTRY_EVENT_DATA_DECODERS[tagged.type];
  if (!dataDecoder) {
    fail(`${path}.type`, `unknown event type \`${tagged.type}\``);
  }
  if (!("data" in tagged) || tagged.data === void 0) {
    fail(`${path}.data`, "missing field");
  }
  return { type: tagged.type, data: dataDecoder(tagged.data, `${path}.data`) };
};
const dEventLogRecord = dStruct("EventLogRecord", {
  seq: dU64,
...
const dBnsRpcErrorInfo = dStruct("BnsRpcErrorInfo", {
  code: dString,
  message: dString,
...
function bytesToHex$1(bytes2) {
  ...
  for (let i = 0; i < bytes2.length; i++) {
    result += HEX_CHARS[bytes2[i] >> 4] + HEX_CHARS[bytes2[i] & 15];
  }
  return result;
}
...
function normalizeWaitTxOption(option, value) {
...
class BnsClient {
  ...
  decodeResult(method, decoder, value) {
    try {
  ...
  async call(method, params, decoder) {
  ...
  async callNullable(method, params, decoder) {
    const result = await this.callEnvelope(method, params);
    if (result === null) {
      return null;
    }
  ...
  async getDocumentVersion(name, docType, version2) {
    return this.callNullable(METHOD_GET_DOCUMENT_VERSION, { name, doc_type: docType, version: version2 }, dDocumentState);
  }
  ...
  async waitTx(txHash, options = {}) {
    ...
    const intervalMs = normalizeWaitTxOption("intervalMs", options.intervalMs ?? 2e3);
    const timeoutMs = normalizeWaitTxOption("timeoutMs", options.timeoutMs ?? 12e4);
...
send_fn = async function(attempt, expires, delay2, _request2, _response) {
  ...
  if ([301, 302, 307, 308].indexOf(response.statusCode) >= 0) {
    ...
    if (this.retryFunc == null || await this.retryFunc(req, response, attempt)) {
      ...
      return __privateMethod(_b = req.clone(), _send, send_fn).call(_b, attempt + 1, expires, delay3, _request2, response);
    }
  }
  if (this.processFunc) {
    checkSignal(__privateGet(_request2, _signal));
    try {
      ...
      return __privateMethod(_c = req.clone(), _send, send_fn).call(_c, attempt + 1, expires, delay3, _request2, response);
    }
  }
  return response;
};
...
const _Signature = class _Signature {
  ...
  getCanonical() {
    ...
    const v = 55 - this.v;
    const result = new _Signature(_guard$3, this.r, toUint256(s), v);
    if (this.networkV) {
...
const _EnsResolver = class _EnsResolver {
  ...
  async getName() {
    return await __privateMethod(this, _fetch, fetch_fn).call(this, "name(bytes32)");
  }
  /**
...
class AbstractProvider {
  ...
  constructor(_network3, options) {
    ...
    __privateAdd(this, _requestRate, void 0);
    __privateAdd(this, _requestTimes, void 0);
...
perform_fn = async function(req) {
  ...
  if (timeout < 0) {
    const delay2 = __privateMethod(this, _getDelay, getDelay_fn).call(this);
    if (delay2) {
      await stall$3(delay2);
    }
...
let Wallet = _Wallet;
const BNS_EVM_DEFAULT_GAS_LIMIT = 3000000n;
const BNS_EVM_DEFAULT_MAX_FEE_PER_GAS = 2000000000n;
const BNS_EVM_DEFAULT_MAX_PRIORITY_FEE_PER_GAS = 1000000000n;
const BNS_MAX_INLINE_DOCUMENT_BYTES = 4096;
const BNS_DNS_TXT_DEFAULT_TTL = 600;
const BNS_DNS_TXT_DOC_TYPE = "dns_txt";
const MAX_U32 = 4294967295;
const MAX_U64 = (1n << 64n) - 1n;
const BNS_PUBLISH_DOCUMENT_ABI = [
...
const UNSET_PRINCIPAL = [0, "0x"];
const STORAGE_TYPE_INLINE = encodeBytes32String("inline");
class BnsEvmTxError extends Error {
  constructor(code, message) {
    super(message);
...
function toConfigBigInt(field, value, max) {
  let parsed;
  try {
    parsed = BigInt(value);
  } catch {
  ...
  if (parsed < 0n || max !== void 0 && parsed > max) {
    throw new BnsEvmTxError("INVALID_CONFIG", `${field} is out of range`);
  }
  return parsed;
}
function normalizeBytes32(field, value) {
  if (!value) {
    return ZeroHash;
  }
  const bytes2 = getBytes(value);
...
function canonicalJsonBytes(value) {
  ...
  const bytes2 = toUtf8Bytes(canonical);
  if (bytes2.length === 0 || bytes2.length > BNS_MAX_INLINE_DOCUMENT_BYTES) {
    throw new BnsEvmTxError(
      "INVALID_DOCUMENT",
      `inline document is ${bytes2.length} bytes, max ${BNS_MAX_INLINE_DOCUMENT_BYTES}`
    );
  }
  return bytes2;
}
...
function validateDnsTxtRecord(record, path) {
  if (!record || typeof record !== "object" || Array.isArray(record)) {
    throw new BnsEvmTxError("INVALID_DOCUMENT", `${path} must be an object`);
  }
  const source = record;
  if (!Number.isInteger(source.ttl) || source.ttl < 0 || source.ttl > MAX_U32) {
...
class BnsEvmTxBuilder {
  ...
  async buildAndSign(operation, privateKey) {
    const wallet = new Wallet(privateKey);
    const update = operation.type === "publish_document" ? await this.preparePublishedDocument(operation.request) : await this.prepareDnsTxtDocument(operation.request);
  ...
  async preparePublishedDocument(request) {
  ...
  async prepareDnsTxtDocument(request) {
    ...
    return {
      ...
      expectedVersion: (current == null ? void 0 : current.document_state.version) ?? 0,
      inlineDocument: canonicalJsonBytes(records),
      nameState
    };
  }
  async prepareDocument(nameInput, docTypeInput, inlineDocument) {
  ...
  async getCurrentDocument(name, docType) {
    try {
  ...
  encodePublishDocument(update, signerAddress) {
  ...
  ownerAuthority(nameState, signerAddress) {
...
function decodeBnsPublishDocumentCalldata(data) {
  return BNS_INTERFACE.decodeFunctionData("publishDocument", data);
}
class BnsTxExecutorError extends Error {
  constructor(code, route, message) {
    super(message);
...
function isLocalRuntime(runtimeType) {
  return runtimeType === RuntimeType.NodeJS || runtimeType === RuntimeType.AppClient || runtimeType === RuntimeType.AppService;
}
function walletUserHasSnAccount(walletUser) {
...
class BnsTxExecutor {
  constructor(options = {}) {
    this.localExecutionTail = Promise.resolve();
    this.options = options;
    this.evmTxBuilder = options.bnsClient && options.evmConfig ? new BnsEvmTxBuilder(options.bnsClient, options.evmConfig, options.evmProvider) : null;
  }
  async resolveRoute() {
  ...
  async resolveRouteFor(runtimeType, privateKey) {
  ...
  async executeWithSn(operation) {
    ...
    const response = operation.type === "publish_dns_txt" ? await client.publishDnsTxt(operation.request) : await client.publishDocument(operation.request);
    return {
      route: "sn-proxy",
  ...
  async executeLocally(operation, runtimeType, privateKey) {
  ...
  enqueueLocalExecution(execute) {
    const result = this.localExecutionTail.then(execute, execute);
    this.localExecutionTail = result.then(() => void 0, () => void 0);
    return result;
  }
}
const SN_ROOT_PATH = "/kapi/sn";
const SN_AUTH_PATH = "/kapi/sn/auth";
...
const METHOD_AUTH_REFRESH = "auth.refresh";
const METHOD_AUTH_LOGOUT = "auth.logout";
const METHOD_AUTH_ME = "auth.me";
const METHOD_USER_GET_PROFILE = "user.get_profile";
const METHOD_USER_SET_SELF_CERT = "user.set_self_cert";
...
const METHOD_DEVICE_UPDATE = "device.update";
const METHOD_DEVICE_GET = "device.get";
const METHOD_DEVICE_LIST = "device.list";
const METHOD_DEVICEINFO_RESOLVE_OOD_BY_DID = "deviceinfo.resolve_ood_by_did";
...
class SnClientError extends Error {
  constructor(kind, message, code = null, codeName = null, detail = null) {
    super(message);
  ...
  domainProofInfo() {
    ...
    return parseJsonObject(this.detail);
  }
  // Parsed detail of a BNS write error (1023/1024/1025); null otherwise.
  bnsWriteInfo() {
    if (!this.isSnError("bns_permission_denied") && !this.isSnError("bns_name_already_exists") && !this.isSnError("bns_write_failed")) {
      return null;
    }
    return parseJsonObject(this.detail);
  }
  static fromRpcError(method, error) {
...
function normalizeSnUrl(snUrl, target) {
...
class SnClient {
  ...
  constructor(snUrl, sessionToken = null, options = {}) {
  ...
  async call(channel, method, params) {
    try {
  ...
  async checkActiveCode(activeCode) {
  ...
  async register(req) {
    ...
    if (req.owner_config !== void 0) {
      params.owner_config = req.owner_config;
    }
    if (req.initial_documents !== void 0) {
  ...
  async setSelfCert(selfCert, deviceDid) {
  ...
  async publishDnsTxt(req) {
    const params = { name: req.name, mode: req.mode };
    if (req.request_id !== void 0) {
      params.request_id = req.request_id;
    }
    switch (req.mode) {
...
const sn_client = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  LEGACY_SN_BNS_PATH,
  METHOD_AUTH_CHECK_ACTIVE_CODE,
  METHOD_AUTH_CHECK_USERNAME,
  METHOD_AUTH_LOGIN,
  METHOD_AUTH_LOGOUT,
  METHOD_AUTH_ME,
  METHOD_AUTH_REFRESH,
  METHOD_AUTH_REGISTER,
  METHOD_BNS_PUBLISH_DNS_TXT,
  METHOD_BNS_PUBLISH_DOCUMENT,
  METHOD_DEVICEINFO_RESOLVE_OOD_BY_DID,
  METHOD_DEVICEINFO_RESOLVE_OOD_BY_HOSTNAME,
  METHOD_DEVICE_GET,
  METHOD_DEVICE_LIST,
  METHOD_DEVICE_REGISTER,
...
function defaultStoreFetcher(input, init2) {
  if (typeof window !== "undefined" && typeof window.fetch === "function") {
    return window.fetch(input, init2);
  }
  if (typeof globalThis !== "undefined" && typeof globalThis.fetch === "function") {
...
async function materializeFile(file, chunkSize = DEFAULT_CHUNK_SIZE) {
  ...
  if (fileSize <= chunkSize) {
    ...
    const hash2 = sha256Bytes(buf);
    const chunkId = ChunkId.fromMix256Result(buf.length, hash2);
  ...
  while (offset < fileSize) {
    ...
    const hash2 = sha256Bytes(slice);
    const chunkId = ChunkId.fromMix256Result(slice.length, hash2);
...
async function tryCalculateQcidFromFile(file) {
  ...
  const hash2 = sha256Bytes(combined);
  return ChunkId.fromMixHashResult(fileSize, hash2, "qcid").toString();
}
...
async function pickupAndImport(options) {
  ...
  for (let fi = 0; fi < files.length; fi++) {
    ...
    if (shouldGenerateThumbnail(file, options.thumbnails)) {
      ...
      const eager = ((_a2 = options.thumbnails) == null ? void 0 : _a2.eager) !== false;
      if (eager) {
...
async function getImportSessionStatus(sessionId) {
  ...
  for (const [id2, state] of session.objectStates) {
    perObjectProgress[id2] = {
...
async function uploadSingleObject(session, endpoint, state) {
  ...
  for (const [chunkIndex, chunk] of state.chunks.entries()) {
    ...
    await uploadChunkViaTus(
      ...
      (_a2 = session.abortController) == null ? void 0 : _a2.signal
    );
    chunk.uploaded = true;
  }
...
function defaultProxyFetcher(input, init2) {
  if (typeof window !== "undefined" && typeof window.fetch === "function") {
    return window.fetch(input, init2);
  }
  if (typeof globalThis !== "undefined" && typeof globalThis.fetch === "function") {
