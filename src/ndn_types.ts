...
//   - RelationObject
//
// The canonical JSON form (RFC 8785 / JCS) and SHA-256-based ObjId derivation
// are the same as the Rust implementation, so ObjIds generated here are
// byte-equivalent to those generated by ndn-lib for the same input.
...
function ensureValidJcsString(value: string): void {
...
function assertValidJcsPrimitive(value: unknown): void {
    switch (typeof value) {
        case 'string':
...
export function canonicalizeJson(value: any): any {
    ...
    if (Array.isArray(value)) {
        ...
        for (let i = 0; i < value.length; i++) {
            ...
            result[i] = canonicalizeJson(value[i])
        }
        return result
    }
